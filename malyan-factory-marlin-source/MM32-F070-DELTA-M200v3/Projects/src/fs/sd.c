...
void DMA1_SPI_RX(uint8_t *buffer,uint16_t len)
{
    uint8_t temp=0xff;
...
void DMA1_SPI_TX(uint8_t *buffer, uint16_t len)
{
    DMA1_Channel3->CNDTR=len;
...
#define SPI1_DR_ADDRESS                  0x4001300C
    ...
    DMA_InitStructure.DMA_BufferSize = (uint16_t)512;
...
SD_Error SD_ReadMultiBlocks(uint8_t* pBuffer, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  SD_CS_LOW();
  /*
  SD_SendCmd (SD_CMD_READ_MULT_BLOCK, ReadAddr + Offset, 0xFF);
  if (SD_GetResponse(SD_RESPONSE_NO_ERROR)) return  SD_RESPONSE_FAILURE;
  DMA1_SPI_RX(pBuffer,BlockSize*NumberOfBlocks);
  SD_SendCmd( SD_CMD_STOP_TRANSMISSION, 0x00000000, 0xFF);
  SD_CS_HIGH();
  SD_WriteByte(SD_DUMMY_BYTE);
...
/**
  ...
  * @retval None
  */
inline uint8_t SD_WriteByte(uint8_t Data)
{
  return MSD0_spi_read_write(Data);
}
...
/**
  ...
  */
inline uint8_t SD_ReadByte(void)
{
 return MSD0_spi_read_write(SD_DUMMY_BYTE);
}
