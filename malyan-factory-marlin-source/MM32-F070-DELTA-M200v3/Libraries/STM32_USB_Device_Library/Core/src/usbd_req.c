...
* @retval status
*/
USBD_Status  USBD_VendDevReq (USB_CORE_HANDLE  *pdev, USB_SETUP_REQ  *req)
{
   uint16_t len;
  uint8_t *pbuf;
  ...
  switch (req->wIndex) 
  {
   	case USB_DESC_TYPE_OS_FEATURE_EXT_PROPERTIES:
		pbuf = pdev->dev.usr_device->GetExtCompatIDFeatureDescriptor(pdev->dev.speed, &len);
	break;
	case USB_DESC_TYPE_OS_FEATURE_EXT_COMPAT_ID:
		pbuf = pdev->dev.usr_device->GetExtPropertiesFeatureDescriptor(pdev->dev.speed, &len);
	break;
                               ...
                               USB_SETUP_REQ *req)
{
  ...
  case USB_DESC_TYPE_STRING:
    ...
    case USBD_IDX_OS_STR:
              pbuf = pdev->dev.usr_device->GetOSStrDescriptor(pdev->dev.speed, &len);
              break;
